TUWmodel_dual <- function (prec, airt, ep, area=1, param=c(1.2,1.2,2,-2,0,0.9,100,3.3,0.5,9,105,50,2,10,26.5,1,0.8,10), incon=c(50,0,2.5,2.5,0), itsteps=NULL) {
 ...
 dummy <- .Fortran("hbvmodel_dual", itsteps=as.integer(itsteps), nzones=as.integer(nzones), area=area, param=parametri, incon=inconditions,
                    prec=prec, airt=airt, ep=ep, output=output, PACKAGE="TUWmodel")
 names(dummy$param) <- c("SCF","DDF","Tr","Ts","Tm","LPrat","FC","BETA","k0","k1","k2","lsuz","cperc","bmax","croute","fc_skin","f_eta","k_moist")
 names(dummy$incon) <- c("SSM0","SWE0","SUZ0","SLZ0","SSL0")
 dummy$qzones <- t(dummy$output[,1,])
 if (nzones > 1) {
  dummy$q <- apply(dummy$qzones,1,sum)
 } else {
